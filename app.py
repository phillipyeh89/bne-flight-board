...
# ─────────────────────────────────────────────
#  4. UI SETUP & FRAGMENT EXECUTION (V12.14)
# ─────────────────────────────────────────────
...
def _live_dashboard_impl():
    ...
    # never prevent the flight list below from rendering (V12.14 — a broken
    ...
    if not raw_flights:
        ...
        if _err:
            ...
            st.warning(
                "⚠️ No flight data returned. This usually means the AeroDataBox "
                "subscription or API key needs attention — check the RapidAPI "
    ...
    for f in deduped_flights:
        ...
        # Split by data quality (V12.14 fix for the stuck-"On Ground" bug):
    ...
    st.markdown(
        f"<div style='text-align:center; color:{t.text_muted}; font-size:0.65em; margin-top:20px;'>Dev: Phillip Yeh | V12.14</div>",
        unsafe_allow_html=True,
    )
