...
# 滾動視窗：抓取前 1 小時到未來 11 小時的數據 (共 12 小時)
...
with col1:
    ...
    st.caption(f"最後更新：{now_aest.strftime('%H:%M:%S')}")
with col2:
...
processed_flights = []
# 設定早班高消費提醒時間點 (04:10 AM)
alert_threshold = now_aest.replace(hour=4, minute=10, second=0, microsecond=0)
